# Automatisering van NvI-beantwoording voor Zilveren Kruis Wlz-zorginkoop
...
```json
{
  ...
  "answer": "U kunt met uw zorginkoper afspreken welke zorg u mag leveren..."
}
```
...
```mermaid
flowchart LR
    A[NvI Vragen] --> B[Section Matcher]
    C[Inkoopbeleid] --> B
    B --> D[Context Builder]
    D --> E[LLM Generator]
    E --> F[Antwoord + Confidence]
...
```mermaid
flowchart TD
    Q[Vraag met sectie X.Y.Z] --> T1{Tier 1: Direct Match}
    T1 -->|Gevonden| R1[Sectie X.Y.Z + subsecties]
    T1 -->|Niet gevonden| T2{Tier 2: Parent Fallback}
    T2 -->|Gevonden| R2[Sectie X.Y + subsecties]
    T2 -->|Niet gevonden| T3{Tier 3: Keyword Match}
    T3 --> R3[Top 5 secties op keyword-overlap]

    Q2[Vraag zonder sectie] --> T3
...
De zorg voor mensen met een complexe zorgvraag staat al meerdere jaren...

[Sectie 1.2.1] We zetten actief in op de bemiddeling...
In de regel organiseren zorgaanbieders in samenspraak met cliënten...
```
...
flowchart TB
    subgraph Input
        NVI[NvI-GGZ-2024-2026.json<br/>202 vragen]
        IKB[Inkoopbeleid-GGZ-2024-2026.json<br/>112 secties]
    end

    subgraph Processing
        DL[Data Loader]
        SM[Section Matcher<br/>- keyword index<br/>- section index]
        AG[Answer Generator<br/>- OpenAI API<br/>- rate limiting]
    end

    subgraph Output
        OUT[GGZ_answers.json<br/>- antwoorden<br/>- confidence levels<br/>- bronverwijzingen]
    end
...
```mermaid
pie title Betrouwbaarheidsverdeling (n=202)
    "High (44%)" : 89
    "Medium (52%)" : 105
    "Low (3%)" : 6
    "Unanswerable (1%)" : 2
